...
it('card create', function () {
  ...
  cy.get('[data-testid=new-card]').eq(0).click();
  cy.get('[data-testid=new-card-input]').eq(0).type('card 1{enter}');
  cy.wait('@createCard')
...
it('card complete', function () {
  ...
  cy.get('[data-testid=card]').should('be.visible')
  cy.get('[data-testid=card-checkbox]').check();
  cy.wait('@patchCard')
    .its('request.body.completed')
    .should('be.true');
  cy.get('[data-testid=card-checkbox]').uncheck();
  cy.wait('@patchCard')
    .its('request.body.completed')
    .should('be.false');
